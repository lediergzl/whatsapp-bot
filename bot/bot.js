...
// Crear tabla para instancias de bot
db.run(`
...
async function createBotInstance(userPhone) {
  const authPath = path.join(__dirname, 'sessions', userPhone);
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) {
        await startBot(userPhone);
      }
    } else if (connection === 'open') {
      db.run('UPDATE bot_instances SET last_active = CURRENT_TIMESTAMP WHERE user_phone = ?', [userPhone]);
    }
  });
...
async function startBot(userPhone) {
  try {
    const sock = await createBotInstance(userPhone);
    // Aquí va la lógica de manejo de mensajes y comandos
    ...
    console.error(`Error starting bot for ${userPhone}:`, error);
  }
}
...
async function iniciarBot() {
  // Aquí deberías obtener la lista de usuarios con suscripciones activas
  // y llamar a startBot para cada uno de ellos.
  // Ejemplo (reemplazar con tu lógica de obtención de usuarios):
  const users = [{phone: '5551234567'},{phone: '5557654321'}]; // Obtener usuarios de la base de datos

  const botInstances = await Promise.all(users.map(user => startBot(user.phone)));
}
